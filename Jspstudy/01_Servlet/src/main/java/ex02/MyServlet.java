...
public class MyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String name = request.getParameter("name");    // url에 들어가는 파라미터 값이 열루드러옴!!
		String strAge = request.getParameter("age");
		
		// null 처리
		int age = 0;  // (1) 변수 초기화
		if(strAge != null) {
			age = Integer.parseInt(strAge);  // (2) String -> int
		}
		...
		response.setContentType("text/html");   // 사용자에게 html파일보냄
		
		// 2) 응답에 포함되는 한글 처리
		response.setCharacterEncoding("UTF-8");
		
		// 1) MIME-TYPE + 2) 문자셋 (둘을 동시에 처리)
		response.setContentType("text/html; charset=UTF-8");
		
		// 3) 응답 스트림 생성
		//    (1) 문자 출력 스트림(*Writer)을 생성
		//    (2) response 객체로부터 PrintWriter 객체를 얻을 수 있음
		//        - IOException 예외 처리가 필요한데 이미 처리되어 있음(throws)
		...
		out.flush(); // 출력 스트림에 남아있는 모든 데이터 내보내기 (만약을 위해서)
		out.close();
