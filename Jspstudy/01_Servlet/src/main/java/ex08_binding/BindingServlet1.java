...
public class BindingServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//    2) HttpSession        : 브라우저 종료 전까지 접근해서 사용 가능   (-> 로그인정보 저장)
		//    3) HttpServletRequest : 하나의 요청/응답 사이클 내에서 접근해서 사용 가능
		// 3. 사용 메소드
		//	  1) getAttribute('속성')     : 해당 속성 값 가져오기 (캐스팅이 필요할 수 있음 -> Object타입이기때문)
		...
		ctx.setAttribute("a", 1);   // a속성에 1 저장
		
		// HttpSession
		HttpSession session = request.getSession();  // request를 통해서 만듦
		session.setAttribute("b", 2); // b는 2
		
		// HttpServletRequest
		// 매개변수로 선언되어있기 때문에 그냥 사용 가능
		...
		// out.println("<a href=\"/01_Servlet/BindingServlet2\">이동</a>");  // 새로운 요청이기 때문에 이동하는 순간 c는 사용불가 -> 이 전까지만 c가 3임
		// out.close(); 
		
		//****** 포워드 (내부이동)*******// 중요
		// 1. request를 전달함
		// 2. 컨텍스트 내부 이동이므로 경로 작성 시 컨텍스트 패스(01_Servlet)는 작성하지 않음
		// request값을 한번 더 사용 할 수 있음 -> request를 유지해서 보내줌, c=3살아있음
		// request.getRequestDispatcher("/BindingServlet2").forward(request, response);
	
		//****** 리다이렉트 ******//
		// 1. request를 전달하지 않음
		// 2. 클라이언트 -> 서버로 이동하므로 컨텍스트 패스를 작성해야 함
		// a링크이동, 로케이션이동은 리다이렉트개념 
