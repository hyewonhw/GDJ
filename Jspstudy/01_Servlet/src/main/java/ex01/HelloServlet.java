...
/*
 	...
 	4. URL
 		프로토콜://호스트:포트번호/컨텍스트패스/URLMapping  -> 컨텍스트패스:01_Servlet/URLMapping:서블릿
...
public class HelloServlet extends HttpServlet {  // Servlet은 HttpServlet을 상속 받으면 된다.
	...
	/*
		4. doGet
			1) GET 방식의 요청을 처리
			2) 요청 메소드가 GET인 경우 (주소를 직접 조작해서 이동하는 방식들은 모두 GET방식)
				(1) $.ajax({
						'type': 'GET',
						'url': 'http://localhost:9090/01_Servlet/HelloServlet'
					});
				(2) <a href="http://localhost:9090/01_Servlet/HelloServlet">링크</a>
				(3) <form method="GET" action="http://localhost:9090/01_Servlet/HelloServlet"> 또는 <form>  --> 서브밋할때
