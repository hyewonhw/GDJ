...
public class NaverCaptchaServiceImpl implements NaverCaptchaService {
	...
	// fied
	private final String CLIENT_ID = "tq2feeDwAFErLbNnPmUu";
	private final String CLIENT_SECRET = "w1FLigKs5G";
	...
	public Map<String, String> getCaptchaImage(HttpServletRequest request, String key) {
		...
		try {
			...
			if(con.getResponseCode() == 200) {  // 200 : HttpURLConnection.HTTP_OK
				...
				if(dir.exists() == false) { // dir이 존재하지않는다면
					dir.mkdirs();
				}
				// 저장할 캡차이미지 이름
				...
				int readByte = 0; // 실제로 읽어들인 readByte
				while((readByte = in.read(b)) != -1) { // -1이면끝났다는의미 -1이아니면 안끝남
