...
public class BoardModifyService implements BoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		af.setView(request.getContextPath() + "/board/detail.do?board_no=" + board_no);  // 상세보기로 이동하려면 파라미터 넘겨야함
		af.setRedirect(true);	// UPDATE 이후에는 Redirect -> Redirect 할때는 매핑으로 이동(do로 이동)
		return af;
