...
import com.semi.animal.mapper.GallBrdMapper;
...
public class GallBreServiceImpl implements GallBrdService {
	...
	public void saveGallBrd(HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		// 크로스 사이트 스크립팅 방지
		String gallTitle = securityUtil.preventXSS(request.getParameter("gallTitle"));
		// SHA-256 암호화
		...
		*/
		...
		try {
			...
			if(result > 0) {
				...
				out.println("<script>");
				out.println("alert('대충 게시글 등록 실패했다는 멘트')");
