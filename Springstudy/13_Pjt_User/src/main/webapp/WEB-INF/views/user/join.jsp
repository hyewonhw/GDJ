	...
	function fn_idCheck() {
		$('#id').keyup(function() {
			...
			if(regId.test(idValue) == false){
				...
				return;  // 코드 진행 방지(이후에 나오는 ajax 실행이 안 됨)
			}
			// 중복체크(id가지고 db가서 응답까지 받아와야함(ajax))
			$.ajax({
				/* 요청 */
				type: 'get',
		...
		$('#btn_getAuthCode').click(function(){
			...
			new Promise(function(resolve, reject) { 
				...
				$.ajax({
					...
					success: function(resData){
						// 인증번호 1회용 -> db저장할 필요없음
						// resData에 맵 들어왔고 그 맵 안에 인증코드들어있음
						alert('인증코드를 발송했습니다. 이메일을 확인하세요.');
						// 발송한 인증코드와 사용자가 입력한 인증코드 비교resData.authCode
