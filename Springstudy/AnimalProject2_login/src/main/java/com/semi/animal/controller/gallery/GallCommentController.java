...
public class GallCommentController {
	...
	Map<String, Object> list(HttpServletRequest request) { 
		return gallCommentService.gatCommentList(request); 
	}
	...
	public Map<String, Object> remove(@RequestParam("commentNo") int commentNo){
		return gallCommentService.removeComment(commentNo);
	}
	
	@ResponseBody
	@PostMapping(value="/gall/comment/reply/add", produces="application/json")
	public Map<String, Object> replyAdd(GallCommentDTO reply) {
		return gallCommentService.addReply(reply);
	}
}
