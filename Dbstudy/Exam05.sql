...
-- 연봉 1위 조회 후 행번호 1번 부여 -> ROW NUMBER()
-- INNER JOIN으로 DEPT_NAME 조회???????????????

SELECT DEPT_NAME 
  FROM DEPARTMENT
 WHERE SALARY= (SELECT 
                  FROM EMPLOYEE
                 WHERE (SELECT DEPT_NAME
                          FROM);
...
-- 행 삽입
INSERT INTO 테이블(칼럼1, 칼럼2, ...) VALUES (값1, 값, ...);

-- 행 수정
UPDATE 테이블명 SET 칼럼1 = 값1 , 칼럼2 = 값2, ... WHERE 조건문;
