...
public class UploadServiceImpl implements UploadService {
	...
	public void getUploadList(HttpServletRequest request, Model model) {
		...
		map.put("begin", pageUtil.getBegin() -1);   // mySql은 begin의 시작이 0부터이기 때문에 -1(0부터 10개/ 9부터 10개)
		map.put("recordPerPage", pageUtil.getRecordPerPage());  // begin부터 몇개 방식으로 동작함
		
		// List로 안하고 바로 model에 담아줌
		model.addAttribute("uploadList", uploadMapper.selectUploadList(map));
		model.addAttribute("paging", pageUtil.getPaging("/upload/list"));  // 목록의경로
