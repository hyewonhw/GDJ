...
//		논리 연산자
//		논리 AND : && 모두 true 이면 ture 하나라도 false 이면 false
//		논리 OR  : || 하나라도 true 이면 true 모두 false 이면 false
//		논리 NOT : ! true는 false로 false는 true로

		int a = 10;
		int b = 10;

		boolean result1 = (a == 10) && (b == 10); // true
		boolean result2 = (a == 10) || (b == 10); // ture
...
//		논리 And : false가 발생하면 더 이 상 진행하지 않는다. 최종 결과는 false 이므로.
//		논리 OR  : true가 발생하면 더 이상 진행하지 않는다. 최정 결과는 true 이므로.
		int i = 10;
		int j = 10;
		boolean result9 = (i == 20) && (++j == 11);
		System.out.println(result9); // false
		System.out.println(j); // 11이 아니라 10 이다 진행하지 않았기 때문에

		boolean result10 = (i == 10) || (++j == 11);
		System.out.println(result10);
		System.out.println(j); // 11 이 아니라 10
